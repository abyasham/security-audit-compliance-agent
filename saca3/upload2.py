...
def upload_csvfile():
    ...
    if file_path:
        with open(file_path, 'r', encoding="utf-8") as csv_file:
            ...
            rows = [', '.join(row) for row in reader]  # Join rows into strings
            text = ' '.join(rows)  # Combine all rows into a single string
            ...
            with open("vault.txt", "a", encoding="utf-8") as vault_file:
                for chunk in chunks:
                    # Write each chunk to its own line
                    vault_file.write(chunk.strip() + "\n")  # Two newlines to separate chunks
            print(f"CSV file content appended to vault.txt with each chunk on a separate line.")
